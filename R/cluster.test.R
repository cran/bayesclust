##########################################################################
# Version 8.0.0
# Last edited Mar 09th 2010
##########################################################################
cluster.test <- function(data, nsim=500000, aR=0.4, p=2, k=2, a=2.01, 
  b=0.990099, tau2=1, mcs=0.1, file="", label="data", replications=1) {
...
counts <- rep(0,k)
tmp <- .C("GroupMH", as.single(Y), as.integer(cind),as.integer(k), 
  as.integer(p), as.integer(length(cind)), as.single(means), 
  as.single(variances), as.integer(counts), PACKAGE="bayesclust")
return(list(matrix(tmp[[6]], nrow=p, byrow=TRUE), matrix(tmp[[7]], nrow=p, 
  byrow=TRUE), tmp[[8]][1:k]))
}
#**************************************************************************
...
return(sum((p*lgamma((MV[[3]]/2)+a))-(p/2)*log(MV[[3]]*tau2+1)) - 
  sum(((N/2)+a)*log(N*MV[[2]]+(2/b))))
}
#**************************************************************************
...
if(runif(1)>aR)
  ...
  MHR<-lgnew-lgold+log(Const/n.new + (1-aR)*exp(lgold))-log(Const/n.old + 
    (1-aR)*exp(lgnew))
  MHR<-min(1,exp(MHR))
...
#**************************************************************************
  ...
  cluster.test.list <- vector("list", length=replications)
  
  for (r in 1:replications) {
    Y <- scale(data)			#standardize
    ClusterStat <- vector("numeric", nsim) 
    preConst <- p*a*log(2/b)-p*lgamma(a) 	
    logmarg0 <- p*lgamma(n/2 + a)-(p/2)*log(n*tau2+1) - 
      (n/2 + a)*sum(log(n*diag(var(Y))+ 2/b))
  ...
  #    Const<-aR/(n*(k-1))
    ...
    cluster.test.list[[r]] <- cluster.test.obj
    class(cluster.test.list) <- "cluster.test.reps"
    cat("Replication", r, "done\n", sep=" ")
    flush.console()
  }
  if(replications==1) {
    cluster.test.obj <- cluster.test.list[[1]]
    if (nchar(file)!=0) {
      save(cluster.test.obj, file=file) 
    }
    return(cluster.test.obj)
  }
  else {
    if (nchar(file)!=0) {
      save(cluster.test.list, file=file) 
    }
    return(cluster.test.list)
  }
}
